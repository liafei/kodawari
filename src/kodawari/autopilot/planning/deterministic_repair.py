...
from kodawari.autopilot.planning.planning_validators import normalize_planning_path, path_comparison_is_case_insensitive, planning_path_key
...
def _tasks_by_id(plan: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {_clean_text(task.get('task_id')): dict(task) for task in _plan_tasks(plan) if _clean_text(task.get('task_id'))}
_PATH_MENTION_RE = re.compile('(?<![A-Za-z0-9_./\\\\-])((?:[A-Za-z0-9_.-]+[\\\\/])+[A-Za-z0-9_.-]+\\.py)')
_TEST_PATH_MENTION_RE = re.compile('tests[\\\\/][A-Za-z0-9_.-]+\\.py', re.IGNORECASE)
...
def _parse_method_endpoint(contract: dict[str, Any]) -> tuple[str, str]:
    ...
    endpoint = _clean_text(contract.get('endpoint') or contract.get('path') or contract.get('route') or contract.get('name'))
    if endpoint:
...
def _log_entry(*, rule: str, location: str, before: Any, after: Any, reason: str, **extra: Any) -> dict[str, Any]:
    entry = {'rule': rule, 'location': location, 'before': before, 'after': after, 'reason': reason}
    entry.update(extra)
    return entry
...
def _truncate_invariants(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, task in enumerate(tasks, start=1):
        ...
        log.append(_log_entry(rule='truncate_invariants', location=f'tasks[{index}].invariants', before=invariants, after=truncated, reason='Planner emitted more than 5 invariants; keep the leading 5 before structural validation.'))
    if log:
        plan['tasks'] = tasks
    return log
...
def _synthetic_change_log_finding(task_id: str, reason: str) -> dict[str, Any]:
    return {'severity': 'info', 'category': 'deterministic_repair', 'task_id': task_id, 'description': f'auto:no-finding-required change_log target {task_id}', 'recommendation': reason}

def _add_missing_task_change_log_entries(plan: dict[str, Any], previous_plan: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    log: list[dict[str, Any]] = []
    missing_changes = [(task_id, 'removed') for task_id in sorted(set(previous_tasks) - set(current_tasks))] + [(task_id, 'added') for task_id in sorted(set(current_tasks) - set(previous_tasks))]
    for task_id, action in missing_changes:
        if task_id in declared:
            continue
        entry = {'task_id': task_id, 'fields': ['tasks'], 'reason': f'{action.capitalize()} by deterministic repair to preserve round-to-round planning auditability.'}
        entries.append(entry)
        declared.add(task_id)
        log.append(_log_entry(rule='add_missing_task_change_log_entry', location='change_log', before=None, after=entry, reason=f'Planner revised task set but omitted required {action} task audit entry.', synthetic_finding=_synthetic_change_log_finding(task_id, 'Allow machine-added change_log entry for an added/removed task caused by replanning.')))
    if log:
        plan['change_log'] = entries
    return log

def _normalize_change_log_refs(plan: dict[str, Any], previous_plan: dict[str, Any] | None, previous_findings: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    if not previous_plan:
        return []
    ...
    for index, entry in enumerate(_change_log_entries(plan), start=1):
        ...
        if target in allowed:
            continue
        synthetic_finding = _synthetic_change_log_finding(target, 'Allow machine-accepted change_log target because it is a known task id.')
        log.append(_log_entry(rule='change_log_known_task_ref', location=f'change_log[{index}]', before={'allowed_from_findings': sorted(allowed)}, after={'synthetic_finding_task_id': target}, reason='Known task change_log target can be accepted deterministically without another planner round.', synthetic_finding=synthetic_finding))
        allowed.add(target)
    return log

def _recipe_signature(recipe: dict[str, Any]) -> str:
    normalized = {'surface': _clean_text(recipe.get('surface')).lower(), 'command': _clean_text(recipe.get('command')), 'required': recipe.get('required') if isinstance(recipe.get('required'), bool) else None, 'roots': _string_list(recipe.get('roots'))}
...
def _dedupe_verify_recipes(plan: dict[str, Any], project_root: Path | None) -> list[dict[str, Any]]:
    ...
    for index, recipe in enumerate(recipes, start=1):
        ...
        if raw_signature in raw_seen:
            log.append(_log_entry(rule='dedupe_verify_recipes', location=f'verify_recipes[{index}]', before=recipe, after=None, reason='Duplicate verify recipe removed before structural review.'))
            continue
        ...
        if project_root is not None and roots != existing_roots:
            ...
            log.append(_log_entry(rule='filter_missing_verify_recipe_roots', location=f'verify_recipes[{index}].roots', before=roots, after=existing_roots, reason='Drop verify recipe roots that do not exist in the active workspace.'))
            recipe = updated
        signature = _recipe_signature(recipe)
        if signature in seen:
            log.append(_log_entry(rule='dedupe_verify_recipes', location=f'verify_recipes[{index}]', before=recipe, after=None, reason='Duplicate verify recipe removed after root normalization.'))
            continue
        seen.add(signature)
...
def _infer_layer_owner(task: dict[str, Any], plan: dict[str, Any]) -> str:
    text = ' '.join([_clean_text(task.get('surface')), _clean_text(task.get('task_name')), ' '.join(_string_list(task.get('files_to_change')))]).replace('\\', '/').lower()
...
def _fill_missing_layer_owner(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, task in enumerate(tasks, start=1):
        ...
        log.append(_log_entry(rule='infer_missing_layer_owner', location=f'tasks[{index}].layer_owner', before='', after=inferred, reason='Planner omitted layer_owner; infer the narrow owner from surface, task name, and files_to_change.'))
    if log:
        plan['tasks'] = tasks
    return log
...
def _remove_owned_files_from_read_only(plan: dict[str, Any], project_root: Path | None) -> list[dict[str, Any]]:
    ...
    for task_index, task in enumerate(tasks, start=1):
        owned = {planning_path_key(normalize_planning_path(path), case_insensitive=case_insensitive) for path in _string_list(task.get('files_to_change'))}
        if not owned:
            continue
        for field in ('read_only_files', 'do_not_change'):
            ...
            if not before:
                continue
            after = [path for path in before if planning_path_key(normalize_planning_path(path), case_insensitive=case_insensitive) not in owned]
            if before == after:
                continue
            task[field] = after
            log.append(_log_entry(rule='remove_owned_files_from_read_only', location=f'tasks[{task_index}].{field}', before=before, after=after, reason='A task cannot both own a write path and mark the same path read-only/do-not-change; files_to_change ownership wins.'))
    if log:
        plan['tasks'] = tasks
    return log

def _finding_text(finding: dict[str, Any]) -> str:
    return ' '.join((_clean_text(finding.get(key)) for key in ('source', 'category', 'description', 'recommendation')))
...
def _task_forbids_path(task: dict[str, Any], path_key: str, *, case_insensitive: bool) -> bool:
    ...
    return any((planning_path_key(normalize_planning_path(item), case_insensitive=case_insensitive) == path_key for item in forbidden))

def _append_change_log_entry_for_scope_promotion(plan: dict[str, Any], *, task_id: str, path: str) -> dict[str, Any]:
    entry = {'task_id': task_id, 'fields': ['files_to_change', 'read_only_files'], 'reason': f'Promoted {path} from read_only_files to files_to_change after explicit reviewer scope-conflict finding.'}
    entries = _change_log_entries(plan)
    entries.append(entry)
    plan['change_log'] = entries
    return entry

def _append_change_log_entry_for_verification_only_demote(plan: dict[str, Any], *, task_id: str, paths: list[str]) -> dict[str, Any]:
    entry = {'task_id': task_id, 'fields': ['files_to_change', 'new_files', 'read_only_files', 'related_existing_tests'], 'reason': f"Demoted verification-only/no-op evidence paths from files_to_change to read-only scope: {', '.join(paths)}."}
    entries = _change_log_entries(plan)
    entries.append(entry)
    plan['change_log'] = entries
    return entry
...
def _ensure_verification_only_task_constraints(plan: dict[str, Any], previous_plan: dict[str, Any] | None) -> list[dict[str, Any]]:
    tasks = _plan_tasks(plan)
    if not tasks:
        return []
    log: list[dict[str, Any]] = []
    for task_index, task in enumerate(tasks, start=1):
        ...
        entry = None
        if previous_plan:
            entry = {'task_id': task_id, 'fields': ['execution_constraints'], 'reason': 'Promoted explicit verification-only/no-op booleans to the task execution_constraints.'}
            entries = _change_log_entries(plan)
            entries.append(entry)
            plan['change_log'] = entries
        log.append(_log_entry(rule='ensure_verification_only_task_constraints', location=_zero_based_task_location(task_index, '.execution_constraints'), before=before, after=copy.deepcopy(constraints), reason='Verification-only/no-op mode must be declared on the task card, not only plan-level metadata.', task_id=task_id, change_log_entry=entry))
    if log:
        plan['tasks'] = tasks
    return log

def _demote_verification_only_write_anchors(plan: dict[str, Any], previous_plan: dict[str, Any] | None) -> list[dict[str, Any]]:
    tasks = _plan_tasks(plan)
    if not tasks:
        return []
    log: list[dict[str, Any]] = []
    for task_index, task in enumerate(tasks, start=1):
        ...
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        change_log_entry = _append_change_log_entry_for_verification_only_demote(plan, task_id=task_id, paths=normalized) if previous_plan else None
        log.append(_log_entry(rule='demote_verification_only_write_anchors', location=_zero_based_task_location(task_index, '.files_to_change'), before={'files_to_change': files_before, 'new_files': new_files_before}, after={'files_to_change': [], 'new_files': [], 'read_only_files': _string_list(task.get('read_only_files')), 'related_existing_tests': _string_list(task.get('related_existing_tests'))}, reason='Explicit verification-only/no-op task must not claim write-owned files; paths become read-only evidence.', task_id=task_id, change_log_entry=change_log_entry, synthetic_finding=_synthetic_change_log_finding(task_id, 'Allow deterministic no-op scope demotion change_log entry.')))
    if log:
        plan['tasks'] = tasks
    return log
_SAFE_NO_EDIT_BOUNDARY = 'No edits to repository-tracked product source/test/docs/config files. Workflow-managed planning/scratch artifacts, pytest temp DB files, run outputs, and pre-existing workspace dirtiness are not product edits.'
_SAFE_DIRTINESS_CHECK = 'Do not use raw VCS workspace dirtiness as a blocking no-op check. Rely on changed_files=[] plus scoped verify output; if a tracked-file comparison is available, compare only repository-tracked product source/test/docs/config files and ignore workflow scratch/tmp artifacts.'
_CD_PREFIX_RE = re.compile('^\\s*(?:cd|chdir|set-location)\\s+(?P<path>(?:\\"[^\\"]+\\"|\'[^\']+\'|[^&;]+?))\\s*(?:&&|;)\\s*(?P<cmd>.+)$', re.IGNORECASE | re.DOTALL)

def _contains_broad_no_edit_text(text: str) -> bool:
    lowered = _clean_text(text).lower()
    if not lowered:
        return False
    broad = ('any file', 'any files', 'every file', 'workspace', 'repository', 'whole repo', 'entire repo', '任何文件', '所有文件', '整个仓库')
...
def _normalize_workspace_relative_verify_commands(plan: dict[str, Any], *, project_root: Path | None) -> list[dict[str, Any]]:
    if project_root is None:
        return []
    log: list[dict[str, Any]] = []
    recipes = _dict_list(plan.get('verify_recipes'))
    for index, recipe in enumerate(recipes):
        ...
        log.append(_log_entry(rule='normalize_workspace_relative_verify_commands', location=f'verify_recipes[{index}].command', before=before, after=after, reason='Verify commands run from project_root; remove hardcoded cd/chained shell prefix.'))
    if log:
    ...
    for task_index, task in enumerate(tasks, start=1):
        ...
        changed_tasks = True
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        log.append(_log_entry(rule='normalize_workspace_relative_verify_commands', location=_zero_based_task_location(task_index), before={field: item['before'] for field, item in task_changes.items()}, after={field: item['after'] for field, item in task_changes.items()}, reason='Task verify commands run from project_root; remove hardcoded cd/chained shell prefix.', task_id=task_id))
    if changed_tasks:
        plan['tasks'] = tasks
    return log
...
def _remove_verification_only_unrequested_smoke_gates(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not tasks:
        return []
    log: list[dict[str, Any]] = []
    allowed_tests = {path for task in tasks for path in _test_paths_in_text(task.get('verify_cmd')) if path}
    if not allowed_tests:
        return []
    approval_before = _dict_list(plan.get('approval_points'))
    approval_after = [item for item in approval_before if not _mentions_unrequested_smoke(json.dumps(item, ensure_ascii=False), allowed_tests)]
    if approval_after != approval_before:
        plan['approval_points'] = approval_after
        log.append(_log_entry(rule='remove_verification_only_unrequested_smoke_gates', location='approval_points', before=approval_before, after=approval_after, reason='Verification-only closure must not add blocking smoke gates outside the requested verify_cmd test set.'))
    recipes_before = _dict_list(plan.get('verify_recipes'))
    recipes_after = [item for item in recipes_before if not _mentions_unrequested_smoke(json.dumps(item, ensure_ascii=False), allowed_tests)]
    if recipes_after != recipes_before:
        plan['verify_recipes'] = recipes_after
        log.append(_log_entry(rule='remove_verification_only_unrequested_smoke_gates', location='verify_recipes', before=recipes_before, after=recipes_after, reason='Verification-only closure must not retain required smoke recipes outside the requested verify_cmd test set.'))
    changed_tasks = False
    for task_index, task in enumerate(tasks, start=1):
        if not verification_only_allows_empty_files(plan, task):
            continue
        before = {'invariants': _string_list(task.get('invariants')), 'coverage_hints': _string_list(task.get('coverage_hints')), 'test_plan': _clean_text(task.get('test_plan'))}
        task['invariants'] = [item for item in before['invariants'] if not _mentions_unrequested_smoke(item, allowed_tests)][:5]
        task['coverage_hints'] = [item for item in before['coverage_hints'] if not _mentions_unrequested_smoke(item, allowed_tests)]
        ...
        after = {'invariants': _string_list(task.get('invariants')), 'coverage_hints': _string_list(task.get('coverage_hints')), 'test_plan': _clean_text(task.get('test_plan'))}
        if before == after:
            continue
        changed_tasks = True
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        log.append(_log_entry(rule='remove_verification_only_unrequested_smoke_gates', location=_zero_based_task_location(task_index), before=before, after=after, reason='Removed unrequested smoke-test requirements from verification-only/no-op task scope.', task_id=task_id))
    if changed_tasks:
        plan['tasks'] = tasks
    return log

def _normalize_verification_only_no_edit_contracts(plan: dict[str, Any], previous_plan: dict[str, Any] | None) -> list[dict[str, Any]]:
    tasks = _plan_tasks(plan)
    if not tasks:
        return []
    log: list[dict[str, Any]] = []
    for task_index, task in enumerate(tasks, start=1):
        if not verification_only_allows_empty_files(plan, task):
            continue
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        before = {'execution_constraints': copy.deepcopy(task.get('execution_constraints')), 'invariants': _string_list(task.get('invariants')), 'forbidden_changes': _string_list(task.get('forbidden_changes'))}
        ...
        entry = None
        if previous_plan:
            entry = {'task_id': task_id, 'fields': sorted(set(changed_fields)), 'reason': 'Normalized verification-only no-edit boundary so workflow scratch/tmp artifacts do not create false blockers.'}
            entries = _change_log_entries(plan)
            entries.append(entry)
            plan['change_log'] = entries
        after = {'execution_constraints': copy.deepcopy(task.get('execution_constraints')), 'invariants': _string_list(task.get('invariants')), 'forbidden_changes': _string_list(task.get('forbidden_changes'))}
        log.append(_log_entry(rule='normalize_verification_only_no_edit_contracts', location=_zero_based_task_location(task_index), before=before, after=after, reason='Verification-only tasks must not use broad workspace dirtiness as a blocking no-op check.', task_id=task_id, change_log_entry=entry, synthetic_finding=_synthetic_change_log_finding(task_id, 'Allow deterministic no-edit-boundary change_log entry.')))
    if log:
        plan['tasks'] = tasks
    return log
...
def _add_verification_only_frontend_read_only_scope(plan: dict[str, Any], *, task_direction: str, project_root: Path | None) -> list[dict[str, Any]]:
    ...
    if not tasks:
        return []
    verification_tasks = [(task_index, task) for task_index, task in enumerate(tasks, start=1) if verification_only_allows_empty_files(plan, task)]
    if not verification_tasks:
        return []
    source_before = {'source_of_truth': _string_list(plan.get('source_of_truth')), 'source_of_truth_canonical': _string_list(plan.get('source_of_truth_canonical'))}
    _append_unique_plan_path_field(plan, 'source_of_truth', [frontend_path])
    source_after = {'source_of_truth': _string_list(plan.get('source_of_truth')), 'source_of_truth_canonical': _string_list(plan.get('source_of_truth_canonical'))}
    log: list[dict[str, Any]] = []
    for task_index, task in verification_tasks:
        before = {'read_only_files': _string_list(task.get('read_only_files')), 'do_not_change': _string_list(task.get('do_not_change')), **source_before}
        _append_unique_path_field(task, 'read_only_files', [frontend_path])
        _append_unique_path_field(task, 'do_not_change', [frontend_path])
        after = {'read_only_files': _string_list(task.get('read_only_files')), 'do_not_change': _string_list(task.get('do_not_change')), **source_after}
        if before == after:
            continue
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        log.append(_log_entry(rule='add_verification_only_frontend_read_only_scope', location=_zero_based_task_location(task_index, '.read_only_files'), before=before, after=after, reason='Task direction includes page/frontend scope; preserve the live frontend file as read-only evidence for no-op closure.', task_id=task_id))
    if log:
        plan['tasks'] = tasks
    return log
_VERIFICATION_ONLY_TRUTH_DOCS = ('docs/任务计划_v1.1.md', 'docs/启动交付与运行手册.md', 'docs/开发交付现状.md', 'docs/prd_coverage_matrix.md')
...
def _add_verification_only_truth_docs_read_only_scope(plan: dict[str, Any], *, project_root: Path | None) -> list[dict[str, Any]]:
    ...
    for task_index, task in verification_tasks:
        ...
        if before == after:
            continue
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        log.append(_log_entry(rule='add_verification_only_truth_docs_read_only_scope', location=_zero_based_task_location(task_index, '.read_only_files'), before=before, after=after, reason='Verification-only closure must preserve project planning/runbook docs as read-only evidence.', task_id=task_id))
    if log:
        plan['tasks'] = tasks
    return log
...
def _add_verification_only_read_later_persistence_scope(plan: dict[str, Any], *, task_direction: str, project_root: Path | None) -> list[dict[str, Any]]:
    ...
    log: list[dict[str, Any]] = []
    for task_index, task in verification_tasks:
        before = {'read_only_files': _string_list(task.get('read_only_files')), 'do_not_change': _string_list(task.get('do_not_change')), 'module_boundaries': boundaries_before, **source_before}
        ...
        after = {'read_only_files': _string_list(task.get('read_only_files')), 'do_not_change': _string_list(task.get('do_not_change')), 'module_boundaries': _dict_list(plan.get('module_boundaries')), **source_after}
        if before == after:
            continue
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        log.append(_log_entry(rule='add_verification_only_read_later_persistence_scope', location=_zero_based_task_location(task_index, '.read_only_files'), before=before, after=after, reason='Read-later persistence is implemented through daily_read_state in edition_assembly.py; keep it as read-only schema evidence.', task_id=task_id))
    if log:
        plan['tasks'] = tasks
    return log
...
def _demote_verification_only_implementation_canonical_truth(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [_log_entry(rule='demote_verification_only_implementation_canonical_truth', location='source_of_truth', before={'source_of_truth': source_before, 'source_of_truth_canonical': canonical_before}, after={'source_of_truth': source_after, 'source_of_truth_canonical': canonical_after}, reason='Verification-only truth should remain docs/tests; implementation files stay as supporting read-only evidence.')]
...
def _promote_verification_only_tests_to_truth(plan: dict[str, Any], project_root: Path | None) -> list[dict[str, Any]]:
    ...
    if not test_paths:
        return []
    before = {'source_of_truth': _string_list(plan.get('source_of_truth')), 'source_of_truth_canonical': _string_list(plan.get('source_of_truth_canonical')), 'tasks_read_only': [_string_list(task.get('read_only_files')) for task in tasks]}
    ...
    if changed_tasks:
        plan['tasks'] = tasks
    after = {'source_of_truth': _string_list(plan.get('source_of_truth')), 'source_of_truth_canonical': _string_list(plan.get('source_of_truth_canonical')), 'tasks_read_only': [_string_list(task.get('read_only_files')) for task in tasks]}
    if before == after:
        return []
    return [_log_entry(rule='promote_verification_only_tests_to_truth', location='source_of_truth_canonical', before=before, after=after, reason='Verification-only canonical truth includes the exact tests that define acceptance.')]
...
def _sync_verification_only_source_docs_read_only_scope(plan: dict[str, Any], *, project_root: Path | None) -> list[dict[str, Any]]:
    ...
    for task_index, task in enumerate(tasks, start=1):
        if not verification_only_allows_empty_files(plan, task):
            continue
        before = {'read_only_files': _string_list(task.get('read_only_files')), 'do_not_change': _string_list(task.get('do_not_change'))}
        _append_unique_path_field(task, 'read_only_files', docs)
        _append_unique_path_field(task, 'do_not_change', docs)
        after = {'read_only_files': _string_list(task.get('read_only_files')), 'do_not_change': _string_list(task.get('do_not_change'))}
        if before == after:
            continue
        task_id = _clean_text(task.get('task_id')) or _zero_based_task_location(task_index)
        log.append(_log_entry(rule='sync_verification_only_source_docs_read_only_scope', location=_zero_based_task_location(task_index, '.read_only_files'), before=before, after=after, reason='Verification-only tasks must keep docs listed as source-of-truth inside the read-only evidence boundary.', task_id=task_id))
    if log:
        plan['tasks'] = tasks
    return log
...
def _ensure_verification_only_work_all_approval(plan: dict[str, Any], *, task_direction: str) -> list[dict[str, Any]]:
    ...
    changed = False
    point = {'name': 'workflow work-all PASS', 'required': True, 'reason': 'The verification-only closure is complete only when workflow work-all finishes PASS after the scoped pytest command succeeds.'}
    ...
    if not changed:
        return []
    return [_log_entry(rule='ensure_verification_only_work_all_approval', location='approval_points', before=approval_points, after={'approval_points': plan.get('approval_points'), 'tasks': tasks}, reason='Task direction requires workflow work-all PASS as an explicit acceptance gate.')]

def _ensure_verification_only_report_approval(plan: dict[str, Any], *, task_direction: str) -> list[dict[str, Any]]:
    text = _clean_text(task_direction).lower()
    ...
    if 'exit code' in existing_text and 'pytest' in existing_text and ('report' in existing_text or 'summary' in existing_text):
        return []
    point = {'name': 'final report includes command exit code pytest summary', 'required': True, 'reason': 'The final report must include the real command executed, exit code, and pytest output summary.'}
    plan['approval_points'] = [*approval_points, point]
    return [_log_entry(rule='ensure_verification_only_report_approval', location='approval_points', before=approval_points, after=plan['approval_points'], reason='Task direction requires a structured final-report evidence contract.')]

def _promote_review_requested_write_paths(plan: dict[str, Any], previous_findings: list[dict[str, Any]] | None, project_root: Path | None) -> list[dict[str, Any]]:
    ...
    for finding in findings:
        ...
        for path in _mentioned_python_paths(finding):
            ...
            for index, task in enumerate(tasks):
                read_only_keys = {planning_path_key(normalize_planning_path(item), case_insensitive=case_insensitive) for item in _string_list(task.get('read_only_files'))}
            ...
            file_keys = {planning_path_key(normalize_planning_path(item), case_insensitive=case_insensitive) for item in files_to_change}
            if path_key in file_keys or len(files_to_change) >= 3:
                continue
            read_only_before = _string_list(task.get('read_only_files'))
            read_only_after = [item for item in read_only_before if planning_path_key(normalize_planning_path(item), case_insensitive=case_insensitive) != path_key]
            ...
            change_log_entry = _append_change_log_entry_for_scope_promotion(plan, task_id=task_id, path=path)
            log.append(_log_entry(rule='promote_review_requested_write_path', location=f'tasks[{task_index + 1}].files_to_change', before={'files_to_change': files_to_change, 'read_only_files': read_only_before}, after={'files_to_change': files_after, 'read_only_files': read_only_after}, reason='Reviewer explicitly identified a files_to_change/read_only_files scope conflict for a single existing file.', path=path, task_id=task_id, change_log_entry=change_log_entry, synthetic_finding=_synthetic_change_log_finding(task_id, 'Allow deterministic scope-conflict change_log entry requested by reviewer.')))
    if log:
        plan['tasks'] = tasks
    return log
...
def _path_aliases(path: str) -> list[str]:
    ...
    match = re.match('(test_t\\d+)', stem.lower())
    if match:
...
def _strip_verify_only_edit_sentences(text: str, protected_paths: list[str]) -> str:
    ...
    if not original or not _mentions_any_path_alias(original, protected_paths):
        return original
    parts = [part.strip() for part in re.split('(?<=[.!?])\\s+|\\n+', original) if part.strip()]
    kept = [part for part in parts if not (_mentions_any_path_alias(part, protected_paths) and _has_edit_verb(part))]
...
def _strip_verify_only_edit_list_items(values: Any, protected_paths: list[str]) -> list[str]:
    ...
    if not items:
        return []
    kept = [item for item in items if not (_mentions_any_path_alias(item, protected_paths) and _has_edit_verb(item))]
...
def _plan_requires_bundled_tests(plan: dict[str, Any]) -> bool:
    ...
    return bool(constraints.get('bundle_implementation_and_tests') or constraints.get('require_test_in_task') or constraints.get('must_bundle_source_and_test'))

def _task_declares_test_edit(task: dict[str, Any]) -> bool:
    text = ' '.join([str(task.get('task_name') or ''), str(task.get('approach') or ''), str(task.get('test_plan') or ''), ' '.join(_string_list(task.get('coverage_hints')))]).lower()
    if not text:
        return False
    test_markers = ('add test', 'add unit test', 'add integration test', 'new test', 'test case', 'edge-case test', 'coverage')
...
def _protect_verify_only_task_direction_paths(plan: dict[str, Any], *, task_direction: str, project_root: Path | None) -> list[dict[str, Any]]:
    ...
    if not tasks:
        return []
    case_insensitive = path_comparison_is_case_insensitive(project_root)
    protected_by_key = {planning_path_key(path, case_insensitive=case_insensitive): path for path in protected}
    log: list[dict[str, Any]] = []
    for task_index, task in enumerate(tasks, start=1):
        ...
        log.append(_log_entry(rule='protect_verify_only_task_direction_paths', location=f'tasks[{task_index}].files_to_change', before=before, after=after, reason="Paths mentioned as 'keep ... passing' in the task direction are verify-only unless explicitly requested as editable.", demoted_to_related_existing_tests=list(demoted)))
    if log:
        plan['tasks'] = tasks
    return log
...
def _normalize_conflicting_api_contracts(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for task_index, task in enumerate(tasks, start=1):
        ...
        keys = sorted(set(api_by_endpoint) | set(provides_by_endpoint), key=lambda item: max(api_by_endpoint.get(item, [-1])), reverse=True)
        changed = False
        for key in keys:
            api_indexes = api_by_endpoint.get(key, [])
            provide_indexes = provides_by_endpoint.get(key, [])
            all_shapes = [_response_shape(api_contracts[index]) for index in api_indexes] + [_response_shape(provides[index]) for index in provide_indexes]
            ...
            before = {'api_contracts': [api_contracts[index] for index in api_indexes], 'provides': [provides[index] for index in provide_indexes]}
            if api_indexes:
                ...
                method, endpoint = key
                api_contracts.append({'method': method, 'endpoint': endpoint, 'response_shape': copy.deepcopy(merged_shape)})
            ...
            changed = True
            method, endpoint = key
            log.append(_log_entry(rule='normalize_api_contract_response_shape', location=f'tasks[{task_index}].api_contracts[{method} {endpoint}]', before=before, after={'method': method, 'endpoint': endpoint, 'response_shape': merged_shape}, reason='Collapse repeated method+endpoint declarations into one validator-compatible response_shape.'))
        if changed:
...
def _files_by_key(task: dict[str, Any], *, case_insensitive: bool) -> dict[str, str]:
    return {planning_path_key(normalize_planning_path(path), case_insensitive=case_insensitive): normalize_planning_path(path) for path in list(task.get('files_to_change') or []) if _clean_text(path)}

def _serialize_parallel_file_conflicts(plan: dict[str, Any], project_root: Path | None) -> list[dict[str, Any]]:
    tasks = _plan_tasks(plan)
    if len(tasks) < 2:
        return []
    case_insensitive = path_comparison_is_case_insensitive(project_root)
        ...
        for index, current in enumerate(tasks):
            ...
            for previous in tasks[:index]:
                ...
                if previous_id not in before:
                    ...
                    log.append(_log_entry(rule='serialize_parallel_file_conflicts', location=f'tasks[{index + 1}].depends_on', before=before, after=after, reason='Tasks that write the same file must run serially before review.', task_id=current_id, depends_on_added=previous_id, shared_files=[current_files[key] for key in shared]))
                    changed = True
                    break
            if changed:
                break
    if log:
        ...
        plan['taskgraph_resolution_log'] = [*existing, *[{'resolution': 'serialized_parallel_write_conflict', 'task_id': item['task_id'], 'depends_on_added': item['depends_on_added'], 'shared_files': list(item['shared_files'])} for item in log]]
    return log

def previous_findings_with_deterministic_refs(previous_findings: list[dict[str, Any]] | None, repair_log: list[dict[str, Any]]) -> list[dict[str, Any]]:
    findings = [dict(item) for item in list(previous_findings or []) if isinstance(item, dict)]
...
def apply_deterministic_repairs(plan: dict[str, Any], *, previous_plan: dict[str, Any] | None=None, previous_findings: list[dict[str, Any]] | None=None, project_root: Path | None=None, task_direction: str='') -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    return (repaired, log)
__all__ = ['apply_deterministic_repairs', 'previous_findings_with_deterministic_refs']
