...
def _git_text(project_root: Path, args: list[str], *, timeout_seconds: int=10) -> str:
    try:
        run = subprocess.run(['git', '-C', str(project_root), *args], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=max(3, int(timeout_seconds)), check=False)
...
def _untracked_content_hash(project_root: Path) -> str:
    ...
    * total bytes read are capped at ``TOTAL_UNTRACKED_READ_BUDGET_BYTES``
    ...
    if over_file_cap or stat_fallback:
        parts.append(f'__cap_degraded__:files={len(names)}|content_hashed={content_hashed}|stat_fallback={stat_fallback}|bytes_read={bytes_read}')
    return _text_hash('\n'.join(parts))
...
def _manifest_roots(project_root: Path, repo_inventory: dict[str, Any]) -> list[Path]:
    ...
    surface_roots = [str(root) for surface in list(repo_inventory.get('surfaces') or []) if isinstance(surface, dict) for root in list(surface.get('roots') or []) if _clean_text(root)]
    candidates = code_roots + test_roots + surface_roots + ['docs', 'tests', 'app', 'src', 'backend', 'web/src', 'frontend/src', 'mobile', 'mobile/www']
    return _existing_dirs(project_root, candidates)

def _repo_manifest(project_root: Path, *, repo_inventory: dict[str, Any], limit: int=DEFAULT_MANIFEST_LIMIT) -> list[str]:
    roots = _manifest_roots(project_root, repo_inventory)
    if not roots:
        roots = [project_root]
    allowed_suffixes = {'.py', '.ts', '.tsx', '.js', '.jsx', '.json', '.yaml', '.yml', '.md', '.html', '.css', '.toml', '.ini', '.cfg', '.txt'}
    values: list[str] = []
    seen: set[str] = set()
    for root in roots:
...
def _tokenize(text: str) -> set[str]:
    tokens: set[str] = set()
    for raw in re.split('[^A-Za-z0-9]+', _clean_text(text).lower()):
...
def _hint_tokens(text: str) -> set[str]:
    ...
    for raw in re.split('[^A-Za-z0-9]+', _clean_text(text).lower()):
        ...
        if len(token) >= 3:
            tokens.add(token)
    return tokens
_PATH_LIKE_RE = re.compile('[A-Za-z0-9_./\\\\-]+\\.[A-Za-z0-9]+')
_GENERIC_PATH_PARTS = {'api', 'app', 'backend', 'docs', 'frontend', 'routes', 'services', 'src', 'tests', 'v1', 'web'}
...
def _mentioned_existing_tests(*, project_root: Path, task_direction: str, prd_excerpt: str, limit: int=3) -> list[str]:
...
def _run_pytest_probe(project_root: Path, args: list[str], *, timeout_seconds: int) -> dict[str, Any]:
    ...
    try:
        completed = subprocess.run(command, cwd=str(project_root.resolve()), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=max(1, int(timeout_seconds)), check=False)
    except subprocess.TimeoutExpired as exc:
        return {'status': 'TIMEOUT', 'returncode': None, 'command': command, 'summary': _probe_output(str(exc.stdout or ''), str(exc.stderr or ''))}
    except OSError as exc:
        return {'status': 'ERROR', 'returncode': None, 'command': command, 'summary': str(exc)}
    return {'status': 'PASS' if completed.returncode == 0 else 'FAIL', 'returncode': int(completed.returncode), 'command': command, 'summary': _probe_output(completed.stdout or '', completed.stderr or '')}

def collect_failing_baseline(*, project_root: Path, task_direction: str, prd_excerpt: str, timeout_seconds: int=30) -> dict[str, Any]:
    ...
    tests = _mentioned_existing_tests(project_root=project_root, task_direction=task_direction, prd_excerpt=prd_excerpt)
    if not tests:
    ...
    if collect['status'] not in {'PASS', 'FAIL'}:
        return {'enabled': True, 'status': 'DIAGNOSTIC', 'target_tests': tests, 'collect': collect, 'run': {}}
    run_args = ['-q', *tests]
    ...
    return {'enabled': True, 'status': status, 'target_tests': tests, 'collect': collect, 'run': run}

def _candidate_paths(*, project_root: Path, manifest: list[str], task_direction: str, prd_excerpt: str, recent_commits: str, uncommitted_changes: str, limit: int=DEFAULT_SNIPPET_LIMIT) -> list[str]:
    tokens = set()
    ...
    selected = _merge_candidate_paths(priority_paths, [path for _, path in scored], limit=max(1, int(limit)))
    if selected:
        return selected
    fallbacks = ['backend/main.py', 'app/main.py', 'src/main.py', 'backend/api/router.py', 'backend/api/v1/router.py']
    existing: list[str] = []
...
def _candidate_snippets(*, project_root: Path, candidates: list[str], max_lines: int=DEFAULT_SNIPPET_LINES) -> list[dict[str, str]]:
    ...
    for rel in candidates:
        ...
        if not snippet:
            continue
        snippets.append({'path': rel, 'reason': 'token_match', 'snippet': snippet})
    return snippets
...
def _collect_instinct_summary(project_root: Path) -> dict[str, Any]:
    ...
    if data is None:
        return {}
    high_confidence = [{'pattern': inst.pattern, 'category': inst.category, 'confidence': inst.confidence, 'count': inst.count, 'explanation': inst.explanation[:120]} for inst in data.learned_instincts or [] if inst.confidence >= 0.75 and (not inst.archived)]
    active_candidates = [{'signature': cand.signature[:120], 'category': cand.category, 'count': cand.count, 'suggested_pattern': cand.suggested_pattern} for cand in data.learning_candidates or [] if cand.count >= 2 and (not cand.promoted)]
    return {'high_confidence_patterns': high_confidence, 'active_candidates': active_candidates[:5], 'total_learned': len([i for i in data.learned_instincts or [] if not i.archived]), 'total_candidates': len(data.learning_candidates or [])}
...
def _collect_telemetry_summary(project_root: Path, feature: str='') -> dict[str, Any]:
    ...
    if not recent:
        return {}
    token_overruns = sum((1 for e in recent if 'TOKEN_BUDGET' in str(e.get('status', ''))))
    max_cycles_hits = sum((1 for e in recent if 'MAX_CYCLES' in str(e.get('status', ''))))
    ...
    return {'recent_runs': len(recent), 'token_overrun_count': token_overruns, 'max_cycles_hit_count': max_cycles_hits, 'avg_cycles': round(avg_cycles_val, 1), 'signals': {'suggest_smaller_scope': token_overruns >= 2 or max_cycles_hits >= 3, 'suggest_more_cycles': avg_cycles_val > 6 and max_cycles_hits == 0}}
...
def _collect_lane_stability_summary(project_root: Path) -> dict[str, Any]:
    ...
    unstable_lanes = [{'lane': lane.get('lane', ''), 'state': lane.get('standing_proof_state', ''), 'pass_rate': lane.get('metrics', {}).get('pass_rate', 0), 'top_root_causes': (lane.get('metrics') or {}).get('top_root_causes', [])[:3]} for lane in lanes if lane.get('standing_proof_state') not in ('stable', 'no_data')]
    return {'unstable_lanes': unstable_lanes, 'top_failure_signatures': list((trend.get('overview') or {}).get('top_failure_signatures', []))[:5], 'overall_status': trend.get('status', '')}
...
def _render_instinct_summary(data: dict[str, Any]) -> str:
    ...
    for p in data['high_confidence_patterns']:
        lines.append(f"- {p['pattern']}: {p.get('explanation', '')[:120]} (confidence={p['confidence']}, seen={p['count']}x, category={p['category']})")
    if data.get('active_candidates'):
        lines.append('')
        lines.append('Emerging patterns (not yet confirmed):')
        for c in data['active_candidates'][:3]:
            lines.append(f"- {c['suggested_pattern']}: {c['signature'][:80]} (seen={c['count']}x)")
    return '\n'.join(lines)
...
def _render_telemetry_summary(data: dict[str, Any]) -> str:
    ...
    if signals.get('suggest_smaller_scope'):
        lines.append(f"SCOPE WARNING: {data.get('token_overrun_count', 0)} token overruns + {data.get('max_cycles_hit_count', 0)} max-cycle hits in last {data.get('recent_runs', 0)} runs. Consider smaller task scope.")
    if signals.get('suggest_more_cycles'):
        lines.append(f"CYCLES: avg {data.get('avg_cycles', 0)} cycles per run. May benefit from higher max_cycles budget.")
    return '\n'.join(lines)
...
def _render_lane_summary(data: dict[str, Any]) -> str:
    ...
    for lane in data['unstable_lanes']:
        ...
        lines.append(f"- {lane['lane']}: state={lane['state']}, pass_rate={lane.get('pass_rate', 0):.0%}, causes=[{causes}]")
    return '\n'.join(lines)
...
def _render_failing_baseline(data: dict[str, Any]) -> str:
    ...
    lines = [f'BASELINE TEST PROBE: status={status}', f"- target_tests: {(', '.join(tests) if tests else '(none)')}"]
...
def _resolve_path_list(paths: list[str], file_manifest: dict[str, list[str]], project_root: Path, new_file_set: set[str]) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    for f in paths:
        ...
        if '/' in normalized and candidates:
            suffix_matches = [item for item in candidates if item == normalized or item.endswith(f'/{normalized}')]
...
def _resolve_task(task: dict[str, Any], file_manifest: dict[str, list[str]], project_root: Path, all_log: list[dict[str, Any]]) -> None:
...
def _resolve_list_field(plan: dict[str, Any], field: str, file_manifest: dict[str, list[str]], project_root: Path, all_log: list[dict[str, Any]]) -> None:
...
def _resolve_roots_field(item: dict[str, Any], file_manifest: dict[str, list[str]], project_root: Path, all_log: list[dict[str, Any]]) -> None:
...
def _resolve_items_roots(items: list[Any], file_manifest: dict[str, list[str]], project_root: Path, all_log: list[dict[str, Any]]) -> None:
    for item in items:
        if isinstance(item, dict):
            _resolve_roots_field(item, file_manifest, project_root, all_log)

def resolve_plan_paths(plan: dict[str, Any], file_manifest: dict[str, list[str]], project_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    metadata: dict[str, Any] = {'auto_resolved': auto_resolved, 'ambiguous': ambiguous, 'total_changes': len(auto_resolved)}
    return (plan, metadata)

def _build_canonical_hints(file_manifest: dict[str, list[str]], task_direction: str='', mention_texts: list[str] | None=None, max_entries: int=100) -> str:
    ...
    other_basenames: list[str] = [basename for basename in sorted(file_manifest.keys()) if basename not in priority_set and any(('/' in p for p in file_manifest[basename]))]
    lines: list[str] = []
...
def _filter_repo_manifest_by_keywords(repo_files: list[str], task_direction: str='', mention_texts: list[str] | None=None, max_files: int=50) -> list[str]:
...
def _looks_like_config_or_manifest(file_path: str) -> bool:
    ...
    config_names = {'readme.md', 'claude.md', 'pyproject.toml', 'setup.py', 'setup.cfg', 'requirements.txt', 'package.json', 'tsconfig.json', '.env.example', 'docker-compose.yml', 'dockerfile'}
    basename = normalized.rsplit('/', 1)[-1]
    return basename in config_names or normalized.endswith('/__init__.py')

def _filter_git_diff_by_keywords(diff_text: str, task_direction: str='', max_chars: int=5000) -> str:
        ...
        if line.startswith('diff --git '):
            ...
            if len(parts) >= 4:
                ...
                if task_tokens:
                    ...
                    path_lower = file_path.lower()
                    include_current_file = ('/src/' in path_lower or path_lower.startswith('src/')) and '.py' in path_lower or _looks_like_config_or_manifest(file_path)
            else:
                include_current_file = False
...
def collect_planning_context(*, project_root: Path, repo_inventory: dict[str, Any], prd_path: Path | None=None, task_direction: str='', feature: str='', max_lines_per_doc: int=DEFAULT_MAX_LINES_PER_DOC, planning_dir: Path | None=None) -> dict[str, Any]:
    root = project_root.resolve()
    ...
    candidates = _candidate_paths(project_root=root, manifest=manifest, task_direction=task_direction, prd_excerpt=prd_excerpt, recent_commits=recent_commits, uncommitted_changes=uncommitted_changes, limit=DEFAULT_SNIPPET_LIMIT)
    ...
    failing_baseline = collect_failing_baseline(project_root=root, task_direction=task_direction, prd_excerpt=prd_excerpt)
    precondition_hint = _load_precondition_replan_hint(planning_dir)
    payload: dict[str, Any] = {'schema_version': 'planning.context.v1', 'task_direction': _clean_text(task_direction), 'project_root': str(root), 'prd_path': str(prd_path.resolve()) if prd_path is not None and prd_path.exists() else '', 'claude_md': claude_md, 'task_plans': task_plans, 'dev_status': dev_status, 'prd_coverage': prd_coverage, 'prd_excerpt': prd_excerpt, 'readme_excerpt': readme_excerpt, 'recent_commits': recent_commits, 'uncommitted_changes': uncommitted_changes, 'precondition_replan_hint': precondition_hint, 'repo_inventory_summary': {'archetype': _clean_text(repo_inventory.get('archetype')), 'code_roots': [str(item) for item in list(dict(repo_inventory.get('project_layout') or {}).get('code_roots') or []) if _clean_text(item)], 'capabilities': [str(item) for item in list(repo_inventory.get('capabilities') or []) if _clean_text(item)]}, 'repo_manifest': {'files': manifest}, 'file_manifest': file_manifest, 'candidate_snippets': snippets, 'instinct_risk_zones': instinct_risk_zones, 'telemetry_summary': telemetry_summary, 'lane_stability': lane_stability, 'failing_baseline': failing_baseline, 'hashes': {'task_direction': _text_hash(task_direction), 'prd': _text_hash(prd_excerpt), 'claude_md': _text_hash(claude_md), 'docs': _text_hash('\n'.join([task_plans, dev_status, prd_coverage, readme_excerpt])), 'repo_inventory': _json_hash(dict(repo_inventory or {})), 'git_head': _text_hash(git_head), 'git_diff_stat': _text_hash(uncommitted_changes), 'untracked_files': _untracked_content_hash(root), 'instinct_risk_zones': _text_hash(json.dumps(instinct_risk_zones, sort_keys=True)), 'telemetry_signals': _text_hash(json.dumps(telemetry_summary, sort_keys=True)), 'lane_stability': _text_hash(json.dumps(lane_stability, sort_keys=True)), 'failing_baseline': _text_hash(json.dumps(failing_baseline, sort_keys=True))}}
...
def compute_input_fingerprint(context: dict[str, Any]) -> str:
    ...
    pieces = [_clean_text(context.get('task_direction')), _clean_text(context.get('prd_path')), _clean_text(hashes.get('task_direction')), _clean_text(hashes.get('prd')), _clean_text(hashes.get('claude_md')), _clean_text(hashes.get('docs')), _clean_text(hashes.get('repo_inventory')), _clean_text(hashes.get('git_head')), _clean_text(hashes.get('git_diff_stat')), _clean_text(hashes.get('untracked_files')), _clean_text(hashes.get('instinct_risk_zones')), _clean_text(hashes.get('telemetry_signals')), _clean_text(hashes.get('lane_stability')), _clean_text(hashes.get('failing_baseline'))]
    digest = hashlib.sha256('|'.join(pieces).encode('utf-8', errors='replace')).hexdigest()
    return f'sha256:{digest}'
_MIGRATION_FILENAME_RE = re.compile('^(\\d{8})_(\\d{3})_(.+)\\.sql$', re.IGNORECASE)
...
def _render_field_evidence_lines(evidence: dict[str, Any]) -> list[str]:
    ...
    out = [f'      structural DDL evidence: CREATE TABLE matches={create_count}, ALTER TABLE ADD COLUMN matches={alter_count}']
    if code_files:
        sample = ', '.join(code_files[:3])
        suffix = ' (...)' if len(code_files) > 3 else ''
        out.append(f'      code-string mentions ({len(code_files)} files, NOT proof of column existence): {sample}{suffix}')
...
def _render_prereq_task_template(missing_field: list[str], project_root: Path | None) -> list[str]:
    ...
    return ['', 'Concrete prereq task shape — emit one task like this BEFORE the dependent task:', 'CRITICAL: copy the suggested file path verbatim. Do NOT substitute placeholders', 'like <NEXT> or <DATE>; the runtime path guard rejects placeholder strings.', '```json', '{', f'  "task_id": "T0_ADD_{sample_column.upper()}",', f'  "task_name": "Add {sample} via migration",', '  "why_this_layer": "schema migration unblocks downstream task",', f'  "files_to_change": ["{suggested_path}"],', f'  "new_files": ["{suggested_path}"],', '  "invariants": ["additive migration only; existing rows nullable"],', f'  "test_plan": "PRAGMA table_info({sample_table}) shows the column",', '  "requires": []', '}', '```']
...
def _render_precondition_hint(hint: dict[str, Any], *, project_root: Path | None=None) -> str:
    ...
    lines: list[str] = ['A previous autopilot run hit BLOCKED_BY_PRECONDITION on this feature.', 'You MUST insert a prerequisite schema/migration/symbol task BEFORE the dependent task.', 'Do NOT mark these as `existing: true` again — readiness verified they are absent from the DDL.']
    if missing_field:
...
def _render_review_triggered_evidence(packs: list[Any]) -> str:
    ...
    for pack in packs[:3]:
        ...
        for request in list(pack.get('requests') or [])[:4]:
            ...
            rendered.append(f'- finding_id={finding_id} round={round_number} category={category} status={status}\n  reviewer_claim: {claim}')
    ...
    if not rendered:
        return ''
    return 'Reviewer raised factual objections. For each finding_id below add an evidence_resolutions entry with status, evidence_refs, and rationale. Allowed status values: finding_refuted (cite refs from this pack to disprove the claim), finding_supported (accept the finding and revise the plan accordingly — closing the request), ambiguous (only when evidence is genuinely inconclusive; a finding that stays ambiguous for 2 consecutive rounds will escalate the run).\n' + '\n'.join(rendered)
...
def render_context_for_prompt(context: dict[str, Any], *, max_chars: int=DEFAULT_MAX_CHARS, planning_round: int=1, incremental_mode: bool=False) -> str:
    ...
    """
    precondition_hint_text = _render_precondition_hint(dict(context.get('precondition_replan_hint') or {}), project_root=_coerce_project_root(context.get('project_root')))
    if incremental_mode:
        if planning_round == 1:
            sections = [_Section('task_direction', 'Task Direction', _clean_text(context.get('task_direction')), 0), _Section('precondition_replan_hint', 'PRECONDITION REPLAN HINT (override task ordering)', precondition_hint_text, 0.1), _Section('canonical_path_hints', 'Canonical Path Hints — use full paths in files_to_change', _build_canonical_hints(dict(context.get('file_manifest') or {}), task_direction=_clean_text(context.get('task_direction')), mention_texts=[_clean_text(context.get('task_direction')), _clean_text(context.get('claude_md')), _clean_text(context.get('task_plans'))], max_entries=100)[:4000], 0.5), _Section('prd_excerpt', 'PRD Excerpt (AUTHORITATIVE — the single source of truth for routes, function signatures, error contracts, and response shapes; do NOT invent alternatives)', _clean_text(context.get('prd_excerpt')), 0.6), _Section('claude_md', 'CLAUDE.md', _clean_text(context.get('claude_md')), 1), _Section('task_plans', 'Task Plans', _clean_text(context.get('task_plans')), 2)]
        elif planning_round == 2:
            sections = [_Section('task_direction', 'Task Direction', _clean_text(context.get('task_direction')), 0), _Section('precondition_replan_hint', 'PRECONDITION REPLAN HINT (override task ordering)', precondition_hint_text, 0.1), _Section('canonical_path_hints', 'Canonical Path Hints — use full paths in files_to_change', _build_canonical_hints(dict(context.get('file_manifest') or {}), task_direction=_clean_text(context.get('task_direction')), mention_texts=[_clean_text(context.get('task_direction')), _clean_text(context.get('claude_md')), _clean_text(context.get('task_plans'))], max_entries=100)[:4000], 0.5), _Section('prd_excerpt', 'PRD Excerpt (AUTHORITATIVE — the single source of truth for routes, function signatures, error contracts, and response shapes; do NOT invent alternatives)', _clean_text(context.get('prd_excerpt')), 0.6), _Section('claude_md', 'CLAUDE.md', _clean_text(context.get('claude_md')), 1), _Section('task_plans', 'Task Plans', _clean_text(context.get('task_plans')), 2), _Section('dev_status', 'Dev Status', _clean_text(context.get('dev_status')), 3), _Section('readme_excerpt', 'README Excerpt', _clean_text(context.get('readme_excerpt')), 9), _Section('recent_commits', 'Recent Commits', _clean_text(context.get('recent_commits')), 7), _Section('uncommitted_changes', 'Uncommitted Changes', _filter_git_diff_by_keywords(_clean_text(context.get('uncommitted_changes')), task_direction=_clean_text(context.get('task_direction')), max_chars=5000), 8)]
        else:
            sections = [_Section('task_direction', 'Task Direction', _clean_text(context.get('task_direction')), 0), _Section('precondition_replan_hint', 'PRECONDITION REPLAN HINT (override task ordering)', precondition_hint_text, 0.1), _Section('canonical_path_hints', 'Canonical Path Hints — use full paths in files_to_change', _build_canonical_hints(dict(context.get('file_manifest') or {}), task_direction=_clean_text(context.get('task_direction')), mention_texts=[_clean_text(context.get('task_direction')), _clean_text(context.get('claude_md')), _clean_text(context.get('task_plans'))], max_entries=100)[:4000], 0.5), _Section('prd_excerpt', 'PRD Excerpt (AUTHORITATIVE — the single source of truth for routes, function signatures, error contracts, and response shapes; do NOT invent alternatives)', _clean_text(context.get('prd_excerpt')), 0.6), _Section('claude_md', 'CLAUDE.md', _clean_text(context.get('claude_md')), 1), _Section('task_plans', 'Task Plans', _clean_text(context.get('task_plans')), 2), _Section('dev_status', 'Dev Status', _clean_text(context.get('dev_status')), 3), _Section('prd_coverage', 'PRD Coverage Matrix', _clean_text(context.get('prd_coverage')), 5), _Section('repo_inventory', 'Repo Inventory Summary', json.dumps(context.get('repo_inventory_summary') or {}, ensure_ascii=False, indent=2), 6), _Section('recent_commits', 'Recent Commits', _clean_text(context.get('recent_commits')), 7), _Section('uncommitted_changes', 'Uncommitted Changes', _filter_git_diff_by_keywords(_clean_text(context.get('uncommitted_changes')), task_direction=_clean_text(context.get('task_direction')), max_chars=5000), 8), _Section('instinct_risk', 'Instinct Risk Zones', _render_instinct_summary(dict(context.get('instinct_risk_zones') or {}))[:2000], 8.5), _Section('telemetry_signals', 'Telemetry Signals', _render_telemetry_summary(dict(context.get('telemetry_summary') or {}))[:1000], 8.6), _Section('lane_stability', 'Lane Stability', _render_lane_summary(dict(context.get('lane_stability') or {}))[:1000], 8.7), _Section('failing_baseline', 'Failing Baseline Probe', _render_failing_baseline(dict(context.get('failing_baseline') or {}))[:2200], 0.65), _Section('review_triggered_evidence', 'Review-Triggered Evidence Pack', _render_review_triggered_evidence(list(context.get('review_triggered_evidence') or []))[:6000], 0.66), _Section('readme_excerpt', 'README Excerpt', _clean_text(context.get('readme_excerpt')), 9), _Section('repo_manifest', 'Repo Manifest', '\n'.join((f'- {item}' for item in _filter_repo_manifest_by_keywords(list(dict(context.get('repo_manifest') or {}).get('files') or []), task_direction=_clean_text(context.get('task_direction')), mention_texts=[_clean_text(context.get('task_direction')), _clean_text(context.get('claude_md')), _clean_text(context.get('prd_excerpt'))], max_files=50))), 10), _Section('candidate_snippets', 'Candidate Snippets', '\n\n'.join((f"[{item.get('path', '')}]\n{item.get('snippet', '')}" for item in list(context.get('candidate_snippets') or [])[:8] if isinstance(item, dict))), 0.7)]
    else:
        sections = [_Section('task_direction', 'Task Direction', _clean_text(context.get('task_direction')), 0), _Section('precondition_replan_hint', 'PRECONDITION REPLAN HINT (override task ordering)', precondition_hint_text, 0.1), _Section('canonical_path_hints', 'Canonical Path Hints — use full paths in files_to_change', _build_canonical_hints(dict(context.get('file_manifest') or {}), task_direction=_clean_text(context.get('task_direction')), mention_texts=[_clean_text(context.get('task_direction')), _clean_text(context.get('claude_md')), _clean_text(context.get('task_plans'))], max_entries=100)[:4000], 0.5), _Section('prd_excerpt', 'PRD Excerpt (AUTHORITATIVE — the single source of truth for routes, function signatures, error contracts, and response shapes; do NOT invent alternatives)', _clean_text(context.get('prd_excerpt')), 0.6), _Section('claude_md', 'CLAUDE.md', _clean_text(context.get('claude_md')), 1), _Section('task_plans', 'Task Plans', _clean_text(context.get('task_plans')), 2), _Section('dev_status', 'Dev Status', _clean_text(context.get('dev_status')), 3), _Section('prd_coverage', 'PRD Coverage Matrix', _clean_text(context.get('prd_coverage')), 5), _Section('repo_inventory', 'Repo Inventory Summary', json.dumps(context.get('repo_inventory_summary') or {}, ensure_ascii=False, indent=2), 6), _Section('recent_commits', 'Recent Commits', _clean_text(context.get('recent_commits')), 7), _Section('uncommitted_changes', 'Uncommitted Changes', _filter_git_diff_by_keywords(_clean_text(context.get('uncommitted_changes')), task_direction=_clean_text(context.get('task_direction')), max_chars=5000), 8), _Section('instinct_risk', 'Instinct Risk Zones', _render_instinct_summary(dict(context.get('instinct_risk_zones') or {}))[:2000], 8.5), _Section('telemetry_signals', 'Telemetry Signals', _render_telemetry_summary(dict(context.get('telemetry_summary') or {}))[:1000], 8.6), _Section('lane_stability', 'Lane Stability', _render_lane_summary(dict(context.get('lane_stability') or {}))[:1000], 8.7), _Section('failing_baseline', 'Failing Baseline Probe', _render_failing_baseline(dict(context.get('failing_baseline') or {}))[:2200], 0.65), _Section('review_triggered_evidence', 'Review-Triggered Evidence Pack', _render_review_triggered_evidence(list(context.get('review_triggered_evidence') or []))[:6000], 0.66), _Section('readme_excerpt', 'README Excerpt', _clean_text(context.get('readme_excerpt')), 9), _Section('repo_manifest', 'Repo Manifest', '\n'.join((f'- {item}' for item in _filter_repo_manifest_by_keywords(list(dict(context.get('repo_manifest') or {}).get('files') or []), task_direction=_clean_text(context.get('task_direction')), mention_texts=[_clean_text(context.get('task_direction')), _clean_text(context.get('claude_md')), _clean_text(context.get('prd_excerpt'))], max_files=50))), 10), _Section('candidate_snippets', 'Candidate Snippets', '\n\n'.join((f"[{item.get('path', '')}]\n{item.get('snippet', '')}" for item in list(context.get('candidate_snippets') or [])[:8] if isinstance(item, dict))), 0.7)]
...
__all__ = ['build_file_manifest', 'collect_failing_baseline', 'collect_planning_context', 'compute_input_fingerprint', 'render_context_for_prompt', 'resolve_plan_paths', '_safe_avg', '_collect_instinct_summary', '_collect_telemetry_summary', '_collect_lane_stability_summary']
