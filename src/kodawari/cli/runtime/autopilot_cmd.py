...
from kodawari.autopilot.planning.complexity_detector import ComplexityInput, detect_complexity, model_advisor_tier_classifier
...
from kodawari.autopilot.planning.lane_observation import ActualRunSignals, build_lane_observation, to_learning_event, write_lane_observation
from kodawari.autopilot.engine.workflow_policy import UserPolicyOverrides, resolve_workflow_policy
...
from kodawari.cli.runtime.autopilot_decision_bridge import build_release_decision_spec, resolve_decision_payload_for_spec
from kodawari.cli.runtime.autopilot_loop import capture_autopilot_worktree_preflight, persist_command_runtime, resolve_reliable_changed_files
from kodawari.cli.runtime.autopilot_release_flow import MERGED_CONTRACT_VERSION, REQUIRED_PLANNING_ARTIFACTS, build_autopilot_payload, maybe_pause_for_planning_decision, maybe_run_release_tail
...
from kodawari.cli.runtime.autopilot_runtime_flow import build_default_task, bootstrap_command_runtime, execute_autopilot_runtime, explicit_planning_input_requested, load_or_init_state as _load_or_init_state, read_requirements_text, resolve_planning_paths, resolve_requirements_file
...
from kodawari.cli.evidence.self_repair import SELF_REPAIR_FILENAME, build_self_repair_proposal, write_self_repair_markdown, write_self_repair_proposal
from kodawari.cli.evidence.self_repair_execute import ENV_AUTO_EXECUTE as _SELF_REPAIR_ENV_AUTO_EXECUTE, ENV_DEPTH as _SELF_REPAIR_ENV_DEPTH, ENV_ENABLED as _SELF_REPAIR_ENV_ENABLED, execute_self_repair_proposal, write_execution_record as write_self_repair_execution_record
...
__all__ = ['MERGED_CONTRACT_VERSION', 'REQUIRED_PLANNING_ARTIFACTS', '_load_or_init_state', 'run_autopilot_command', 'run_autopilot_release_tail']

def _emit_unavailable_payload(missing_module: str | None) -> int:
    print(json.dumps({'status': 'unavailable', 'message': 'autopilot engine is not restored yet', 'missing_module': missing_module}, ensure_ascii=False, indent=2))
    return 2
...
def _restore_legacy_active_task(*, args: argparse.Namespace, feature: str, requirements_text: str, state: Any, planning_snapshot: Any) -> None:
    has_model_conversation = bool(planning_snapshot is not None and str(dict(planning_snapshot.artifacts).get('PLANNING_CONVERSATION.json') or '').strip())
    if planning_snapshot is not None and (has_model_conversation or explicit_planning_input_requested(args)):
        return
    task_label, _ = resolve_primary_task(args, feature=feature, requirements_text=requirements_text, default_task_factory=build_default_task, planning_task_factory=build_default_task)
    state.active_task = task_label

def _resolve_prebootstrap_context(args: argparse.Namespace) -> tuple[str, Path, str, Any, Any]:
    ...
    if pre_requested_tier == 'auto':
        ...
        pre_changed_files = ()
    decision, policy = _resolve_tier_and_policy(args=args, feature=pre_feature, requirements_text=pre_requirements_text, changed_files=pre_changed_files, project_root=pre_project_root)
    if pre_requested_tier == 'auto' and (not _is_legacy_autopilot_mode()):
        import sys as _sys
        print(f'[autopilot] auto-detected tier={decision.tier} (source={decision.source}). Pass --tier {decision.tier} to silence this message, --tier heavy for legacy-compatible behavior, or set WORKFLOW_AUTOPILOT_LEGACY=1 to fully opt out.', file=_sys.stderr)
    return (pre_feature, pre_project_root, pre_requirements_text, decision, policy)

def _maybe_apply_escalation_resume(*, args: argparse.Namespace, feature: str, project_root: Path) -> int | None:
    ...
    """
    try:
        from kodawari.autopilot.escalation.resume import apply_pending_resume, detect_pending_resume
    except ImportError:
        return None
    ...
    outcome = apply_pending_resume(planning_dir=planning_dir, feature=feature, project_root=project_root, autopilot_args=autopilot_args)
...
def _bootstrap_autopilot_runtime(args: argparse.Namespace, runtime: Any, policy: Any) -> tuple[dict[str, Any], bool] | None:
    ...
    try:
        planning_env_args = argparse.Namespace(tier=getattr(policy, 'effective_tier', getattr(args, 'tier', 'auto')), plan_reviewer_model=str(getattr(args, 'plan_reviewer_model', '') or ''))
        with _planning_env_override_for_tier(planning_env_args):
            command_runtime = bootstrap_command_runtime(args=args, runtime=runtime)
...
_WALL_CLOCK_EXIT_CODE = 124

class _WallClockBudgetExceeded(Exception):
    """Raised by the wall-clock watchdog when budget is exhausted."""

def _start_wall_clock_watchdog(*, budget_seconds: int, on_expire: 'threading.Event') -> threading.Thread | None:
...
def _write_abort_report(*, planning_dir: Path, budget_seconds: int, elapsed_seconds: float, feature: str, cause: str) -> Path | None:
    ...
    try:
        ...
        payload = {'schema_version': 'abort_report.v1', 'generated_at': datetime.now(timezone.utc).isoformat(), 'feature': feature, 'cause': cause, 'budget_seconds': int(budget_seconds), 'elapsed_seconds': round(float(elapsed_seconds), 3), 'exit_code': _WALL_CLOCK_EXIT_CODE}
...
def _run_autopilot_with_wall_clock(*, args: argparse.Namespace, budget_seconds: int) -> int:
    ...
    watchdog = _start_wall_clock_watchdog(budget_seconds=budget_seconds, on_expire=completed_event)
    try:
        return _run_autopilot_inner(args)
        ...
        report_path = _write_abort_report(planning_dir=planning_dir, budget_seconds=budget_seconds, elapsed_seconds=elapsed, feature=str(getattr(args, 'feature', '') or ''), cause='wall_clock_budget_exceeded')
        message = f'autopilot wall-clock budget {budget_seconds}s exceeded (elapsed {elapsed:.1f}s); wrote {report_path}'
...
def _run_autopilot_inner(args: argparse.Namespace) -> int:
    ...
    resume_rc = _maybe_apply_escalation_resume(args=args, feature=feature, project_root=project_root)
    if resume_rc is not None:
        return resume_rc
    release_rc = _maybe_resume_existing_passed_workflow_chain_release_tail(args=args, decision=decision, policy=policy)
    if release_rc is not None:
        return release_rc
    try:
        command_runtime, policy_active = _bootstrap_autopilot_runtime(args=args, runtime=runtime, policy=policy)
    except AutopilotPlanningBridgeError as exc:
        release_rc = _maybe_resume_completed_task_graph_release_tail(args=args, error=exc, decision=decision, policy=policy)
        if release_rc is not None:
            return release_rc
    ...
    planning_pause = maybe_pause_for_planning_decision(args=args, planning_dir=command_runtime['planning_dir'], planning_snapshot=command_runtime['planning_snapshot'], policy=policy if command_runtime['policy_active'] else None)
    if planning_pause is not None:
        if hasattr(command_runtime['state'], 'task_claim'):
            ...
            try:
                ...
                pass
        planning_finalization = _maybe_finalize_planning_pause_artifacts(args=args, command_runtime=command_runtime, planning_pause=planning_pause)
    ...
    worktree_preflight = capture_autopilot_worktree_preflight(project_root=command_runtime['project_root'], planning_dir=command_runtime['planning_dir'], feature=command_runtime['feature'])
    plan, run_result, planning_artifacts, workflow_chain, task_cycle_rounds = execute_autopilot_runtime(args=args, engine=command_runtime['engine'], project_root=command_runtime['project_root'], planning_dir=command_runtime['planning_dir'], feature=command_runtime['feature'], requirements_text=command_runtime['requirements_text'], planning_snapshot=command_runtime['planning_snapshot'])
    if workflow_chain is not None:
        chain_path = write_workflow_chain_snapshot(command_runtime['planning_dir'], workflow_chain)
        planning_artifacts['.workflow_chain.json'] = {'path': str(chain_path), 'exists': True}
    else:
        _clear_stale_workflow_chain_snapshot(command_runtime['planning_dir'], planning_artifacts)
    planning_artifacts[WORKTREE_BASELINE_FILENAME] = {'path': str((command_runtime['planning_dir'] / WORKTREE_BASELINE_FILENAME).resolve()), 'exists': (command_runtime['planning_dir'] / WORKTREE_BASELINE_FILENAME).exists()}
    rounds = persist_command_runtime(rounds_path=command_runtime['rounds_path'], run_result=run_result, task_cycle_rounds=task_cycle_rounds)
    reliable_changed_files, changed_files_source = resolve_reliable_changed_files(project_root=command_runtime['project_root'], state=command_runtime['state'], run_result=run_result)
    _restore_legacy_active_task(args=args, feature=command_runtime['feature'], requirements_text=command_runtime['requirements_text'], state=command_runtime['state'], planning_snapshot=command_runtime['planning_snapshot'])
    command_runtime['state'].changed_files = set(reliable_changed_files)
    command_runtime['state'].save(command_runtime['state_path'])
    payload = build_autopilot_payload(args=args, planning_dir=command_runtime['planning_dir'], state_path=command_runtime['state_path'], rounds_path=command_runtime['rounds_path'], plan=plan, run_result=run_result, rounds=rounds, planning_artifacts=planning_artifacts, state=command_runtime['state'], workflow_chain=workflow_chain, changed_files_source=changed_files_source, worktree_preflight=worktree_preflight, planning_snapshot=command_runtime['planning_snapshot'])
    final_decision, final_policy = _resolve_tier_and_policy(args=args, feature=command_runtime['feature'], requirements_text=command_runtime['requirements_text'], changed_files=tuple(reliable_changed_files or ()), project_root=command_runtime['project_root'])
    ...
    run_truth = build_run_truth(project_root=command_runtime['project_root'], planning_dir=command_runtime['planning_dir'], feature=command_runtime['feature'], payload=payload, run_result=run_result, rounds=rounds, state_payload=state_payload, reliable_changed_files=tuple(reliable_changed_files or ()), changed_files_source=changed_files_source)
    ...
    self_repair = _maybe_write_self_repair_artifacts(project_root=command_runtime['project_root'], planning_dir=command_runtime['planning_dir'], run_truth=run_truth)
    if self_repair:
        payload['self_repair'] = self_repair
    _emit_lane_observation(planning_dir=command_runtime['planning_dir'], feature=command_runtime['feature'], decision=decision, payload=payload, rounds=rounds, reliable_changed_files=tuple(reliable_changed_files or ()), project_root=command_runtime['project_root'])
    _cleanup_suppressed_artifacts(planning_dir=command_runtime['planning_dir'], policy=policy, policy_active=bool(command_runtime.get('policy_active')))
    payload, early_rc = maybe_run_release_tail(args=args, command_runtime=command_runtime, payload=payload, run_release_tail=run_autopilot_release_tail)
    _maybe_write_delivery_report(planning_dir=command_runtime['planning_dir'], feature=command_runtime['feature'], payload=payload)
...
def _maybe_warn_unexecuted_tasks(*, args: argparse.Namespace, run_truth: dict[str, Any], payload: dict[str, Any]) -> None:
    ...
    if not ids:
        return
    message = f"plan had {len(ids)} task(s) the autopilot did not execute because task_cycle is disabled (rerun with --task-cycle to complete): {', '.join(ids)}"
...
def _maybe_resume_completed_task_graph_release_tail(*, args: argparse.Namespace, error: AutopilotPlanningBridgeError, decision: Any, policy: Any) -> int | None:
    if error.error_code != 'task_graph_complete':
        return None
    return _maybe_resume_existing_passed_workflow_chain_release_tail(args=args, decision=decision, policy=policy)

def _maybe_resume_existing_passed_workflow_chain_release_tail(*, args: argparse.Namespace, decision: Any, policy: Any) -> int | None:
    policy_active = _policy_should_override_runtime(args)
    ...
    if not _workflow_chain_passed(workflow_chain):
        return None
    base_payload = _completed_task_graph_release_base_payload(feature=feature, planning_dir=planning_dir, workflow_chain=workflow_chain, decision=decision, policy=policy if policy_active else None)
    spec = build_release_decision_spec(feature)
    decision_payload = resolve_decision_payload_for_spec(args=args, planning_dir=planning_dir, planning_snapshot=None, spec=spec, base_payload=base_payload)
    ...
    release_tail = run_autopilot_release_tail(project_root=project_root, planning_dir=planning_dir, feature=feature, config=AutopilotReleaseTailConfig(auto_eval=bool(getattr(policy, 'eval_required', False)) if policy_active and policy is not None else False, risk_profile=str(getattr(policy, 'effective_tier', 'medium') or 'medium')))
    payload, rc = _completed_task_graph_release_payload(base_payload=base_payload, release_tail=release_tail, feature=feature)
    _maybe_write_delivery_report(planning_dir=planning_dir, feature=feature, payload=payload)
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return int(rc or 0)
...
def _completed_task_graph_release_base_payload(*, feature: str, planning_dir: Path, workflow_chain: dict[str, Any], decision: Any, policy: Any) -> dict[str, Any]:
    final_outcome = dict(workflow_chain.get('final_outcome') or workflow_chain.get('chain_final_outcome') or {'status': 'PASS', 'reason': 'ALL_TASKS_COMPLETE'})
    payload = {'status': 'ok', 'entrypoint': 'kodawari autopilot', 'feature': feature, 'planning_dir': str(planning_dir), 'planning_artifact_mode': 'contract_first', 'planning_snapshot': {}, 'run_reason': 'PIPELINE_FINISH', 'workflow_chain': workflow_chain, 'final_outcome': final_outcome, 'task_graph_selection': 'all_tasks_complete'}
...
def _completed_task_graph_release_payload(*, base_payload: dict[str, Any], release_tail: dict[str, Any], feature: str) -> tuple[dict[str, Any], int]:
    ...
    if release_status == 'PASS':
        payload.update(build_interaction_snapshot(decision_pending=False, decision_kind='release_approval', decision_id=f'{feature}:release_approval', decision_request_present=False, final_status='PASS', stop_reason='PASS', blocked=False, is_terminal=True))
        return (payload, 0)
    ...
    payload.update(build_interaction_snapshot(decision_pending=False, decision_kind='release_approval', decision_id=f'{feature}:release_approval', decision_request_present=False, final_status='BLOCKED', stop_reason='BLOCKED', blocked=True, is_terminal=True))
    return (payload, 1)

def _clear_stale_workflow_chain_snapshot(planning_dir: Path, planning_artifacts: dict[str, dict[str, Any]]) -> None:
    ...
    if chain_path.exists():
        try:
            ...
            pass
    planning_artifacts['.workflow_chain.json'] = {'path': str(chain_path.resolve()), 'exists': chain_path.exists()}

def _emit_planning_bridge_error(*, args: argparse.Namespace, error: AutopilotPlanningBridgeError) -> int:
    project_root = Path(args.project_root).resolve()
    planning_dir, _, _ = resolve_planning_paths(project_root, str(args.feature))
    payload = {'status': 'blocked', 'entrypoint': 'kodawari autopilot', 'feature': str(args.feature), 'planning_dir': str(planning_dir), 'error': str(error), 'error_code': error.error_code, 'remediation': list(error.remediation), 'details': dict(error.details), 'next_action': 'Fix the planning input or artifact issue, then rerun autopilot.'}
    if error.error_code == 'context_scout_user_decision_required':
        payload.update(build_interaction_snapshot(decision_pending=True, decision_kind='context_scout', decision_id=str(error.details.get('selected_tier') or ''), decision_request_present=True))
    if error.error_code in {'prd_missing', 'prd_required'}:
        payload.update(build_interaction_snapshot(decision_pending=False, decision_request_present=False, environment_error_code=error.error_code, environment_blocking_reason=str(error)))
    planning_finalization = _maybe_finalize_planning_bridge_error_artifacts(args=args, planning_dir=planning_dir, project_root=project_root, error=error)
    if planning_finalization:
        payload['planning_finalization'] = planning_finalization
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return 2

def _maybe_finalize_planning_bridge_error_artifacts(*, args: argparse.Namespace, planning_dir: Path, project_root: Path, error: AutopilotPlanningBridgeError) -> dict[str, Any]:
    ...
    if planning_status not in {'escalation_required', 'precondition_blocked', 'error'}:
        return {}
    return _finalize_planning_blocked_artifacts(args=args, project_root=project_root, planning_dir=planning_dir, state_payload={}, fallback_reason=error.error_code)

def _maybe_finalize_planning_pause_artifacts(*, args: argparse.Namespace, command_runtime: dict[str, Any], planning_pause: dict[str, Any]) -> dict[str, Any]:
    ...
    state_payload = state_to_dict() if callable(state_to_dict) else {}
    return _finalize_planning_blocked_artifacts(args=args, project_root=Path(command_runtime['project_root']), planning_dir=Path(command_runtime['planning_dir']), state_payload=state_payload, fallback_reason='PLANNING_ESCALATION_REQUIRED')

def _finalize_planning_blocked_artifacts(*, args: argparse.Namespace, project_root: Path, planning_dir: Path, state_payload: dict[str, Any], fallback_reason: str) -> dict[str, Any]:
    ...
    reason = str(planning_failure.get('error_code') or '').strip() or str(planning_failure.get('reason') or '').strip() or str(escalation.get('termination_reason') or '').strip() or str(fallback_reason or '').strip() or 'PLANNING_ESCALATION_REQUIRED'
    run_result = {'reason': reason, 'blocking_reason': reason, 'final_status': 'BLOCKED', 'unified_status': {'final_status': 'BLOCKED', 'stage_status': 'BLOCKED'}}
    payload = {'run_reason': reason, 'blocking_reason': reason, 'final_status': 'BLOCKED', 'unified_status': {'final_status': 'BLOCKED', 'stage_status': 'BLOCKED'}}
    run_truth = build_run_truth(project_root=project_root, planning_dir=planning_dir, feature=str(getattr(args, 'feature', '') or planning_dir.name), payload=payload, run_result=run_result, rounds=[], state_payload=state_payload, reliable_changed_files=(), changed_files_source='none')
    write_run_truth(planning_dir, run_truth)
    summary: dict[str, Any] = {'status': 'finalized', 'run_truth': '.run_truth.json', 'run_reason': reason}
    self_repair = _maybe_write_self_repair_artifacts(project_root=project_root, planning_dir=planning_dir, run_truth=run_truth)
    if self_repair:
...
def _apply_policy_to_args(*, args: argparse.Namespace, policy) -> None:
    ...
    if _DEFAULT_SELF_REVIEW_BACKEND and (not str(getattr(args, 'self_review_backend', '') or '').strip()):
...
def _emit_lane_observation(*, planning_dir: Path, feature: str, decision, payload: dict[str, Any], rounds: list[dict[str, Any]], reliable_changed_files: tuple[str, ...], project_root: Path | None=None) -> None:
    ...
    if _is_legacy_autopilot_mode():
        return
    try:
        signals = _collect_actual_signals(payload=payload, rounds=rounds, reliable_changed_files=reliable_changed_files, project_root=project_root)
        observation = build_lane_observation(feature=feature, predicted=decision, signals=signals)
        ...
        if observation.mismatch and project_root is not None:
            _ingest_lane_observation_to_instincts(project_root, observation, planning_dir=planning_dir)
    except Exception:
        pass

def _ingest_lane_observation_to_instincts(project_root: Path, observation, *, planning_dir: Path | None=None) -> None:
...
def _collect_actual_signals(*, payload: dict[str, Any], rounds: list[dict[str, Any]], reliable_changed_files: tuple[str, ...], project_root: Path | None=None) -> ActualRunSignals:
    return _lane_signals.collect_actual_signals(payload=payload, rounds=rounds, reliable_changed_files=reliable_changed_files, project_root=project_root, diff_loc_fn=_estimate_diff_loc)

def _estimate_diff_loc(*, project_root: Path | None, changed_files: tuple[str, ...]) -> int:
    return _lane_signals.estimate_diff_loc(project_root=project_root, changed_files=changed_files, subprocess_run=subprocess.run)

def _cleanup_suppressed_artifacts(*, planning_dir: Path, policy: Any, policy_active: bool) -> None:
...
def _build_complexity_input(*, feature: str, task_direction: str, requirements_text: str, changed_files: tuple[str, ...], project_root: Path) -> ComplexityInput:
    ...
    try:
        ...
        learned_hints = ()
    return ComplexityInput(feature=feature, task_direction=task_direction, requirements_text=requirements_text, changed_files=changed_files, task_card_files=task_card_files, layers=layers, learned_hints=learned_hints)

def _resolve_tier_and_policy(*, args: argparse.Namespace, feature: str, requirements_text: str, changed_files: tuple[str, ...], project_root: Path | None=None):
    ...
    inp = _build_complexity_input(feature=str(feature or ''), task_direction=str(getattr(args, 'task', '') or ''), requirements_text=str(requirements_text or ''), changed_files=changed_files, project_root=resolved_root)
    decision = detect_complexity(inp, requested_tier=requested_tier, llm_classifier=model_advisor_tier_classifier)
    lane = lane_for(decision.tier)
    policy = resolve_workflow_policy(decision=decision, lane=lane, overrides=UserPolicyOverrides())
    return (decision, policy)

def _maybe_write_delivery_report(*, planning_dir: Path, feature: str, payload: dict[str, Any]) -> None:
...
def _maybe_write_self_repair_artifacts(*, project_root: Path, planning_dir: Path, run_truth: dict[str, Any]) -> dict[str, Any]:
    try:
        proposal = build_self_repair_proposal(project_root=project_root, planning_dir=planning_dir, run_truth=run_truth)
    except Exception:
    ...
    summary: dict[str, Any] = {'status': 'ready', 'artifact': artifact.name, 'markdown': markdown.name, 'root_cause': str(root_cause.get('code') or '')}
    auto_execution = _maybe_auto_execute_self_repair(planning_dir=planning_dir, proposal_path=artifact)
    if auto_execution:
        summary['auto_execution'] = auto_execution
    return summary

def _maybe_auto_execute_self_repair(*, planning_dir: Path, proposal_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        pass
    return {'status': str(record.get('status') or ''), 'reason': str(record.get('reason') or ''), 'failed_gates': list(record.get('failed_gates') or []), 'spawn_status': (record.get('spawn') or {}).get('status') if isinstance(record.get('spawn'), dict) else '', 'artifact': record.get('artifact')}
